...
sigma = 0.1;
% compute a group of vanish point candidates
candidates = zeros(3,0);
for ii = 1:size(lines,2)-1
    for jj = ii+1:size(lines,2)
        candidates(:,end+1) = cross(lines(:,ii),lines(:,jj));
    end
end
% compute line angles 
lineangles = [];
for ii = 1:size(lines,2)
    line = lines(:,ii);
    slop = -(line(1)/line(2));
    lineangles(:,end+1) = atan(slop);
end
% compute vanish point scores 
maxscore = 0; candidate = -1;
for ii = 1:size(candidates)
    score = 0;
    point = candidates(:,ii);
    px = point(1)/point(3); py = point(2)/point(3);
    for jj = 1:size(lines,2)
        center = centers(:,jj);
        lineptangle = atan(abs(py-center(2))/abs(px-center(1)));
        score = score + abs(line_length(jj)) * exp(-abs(lineptangle-lineangles(jj))/(2*sigma*sigma));
    end
    if score > maxscore
        maxscore = score;
        candidate = ii;
